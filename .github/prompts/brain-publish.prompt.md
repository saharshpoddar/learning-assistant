...
---
name: brain-publish
description: 'Publish a brain/ workspace note to archive/ with tagging, project assignment, and git commit'
agent: copilot
...
${input:file:Path to the file to publish (relative to brain/, e.g. inbox/2026-02-21_draft.md)}
...
1. Read the file at `brain/${input:file}`
...
5. Determine destination: `brain/archive/${input:project}/<YYYY-MM>/<filename>`
...
7. Move the file to `brain/archive/${input:project}/<YYYY-MM>/<filename>`
8. Run: `git add brain/archive/${input:project}/<YYYY-MM>/<filename>`
...
- Never move files outside brain/archive/
